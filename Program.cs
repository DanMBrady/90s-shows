...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            int recentYear = shows.Max(s=> s.StartYear);
            return recentYear;
        }
        ...
        {
            double allRating = shows.Average(s=> s.ImdbRating);
            return allRating;
        }
        ...
        {
            return shows.Where(s=> s.EndYear<2000).ToList();
        }
        ...
        {
            return shows.OrderBy(s=> s.ImdbRating).Skip(1).Reverse().ToList();
        }
        ...
        {
            return shows.Where(s=> s.EndYear>1999).OrderBy(s=>s.EndYear).ToList()[0];
        }
        ...
        {
            return shows.Where(s=>s.Genres.Contains("Drama")).OrderByDescending(s=>s.ImdbRating).ToList()[0];
        }
        ...
        {
            return shows.Where(s=>(s.EndYear-s.StartYear)>10 & s.ImdbRating < 8).OrderBy(s=>s.Name).ToList()[0];
        }

        // 21. Return the show with the most words in the name.
        static Show WordieastName(List<Show> shows)
        {
            return shows.OrderByDescending(s=>s.Name.Count()).ToList()[0];
        }
